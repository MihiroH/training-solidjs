...
function App() {
  ...
  const currentPageInfo = createMemo(() => pagesInfo.filter((info) => info.path === pathname())[0])
  const isNotFoundPage = createMemo(() => !currentPageInfo())
  const isTopPage = createMemo(() => {
    return pathname() === pagesInfo.filter((info) => info.name === 'top')[0].path
  })
    ...
    <>
      <Show when={!isNotFoundPage()} fallback={<Routes />}>
        ...
        <div class={styles.section}>
          <SectionHeader backgroundImage={currentPageInfo().headerBackGroundImage} title={currentPageInfo().title} />
          <Routes />
        </div>
      </Show>
    </>
